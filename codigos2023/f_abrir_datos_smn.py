...
def abrir_datos_smn(ruta_a_archivos, nombre_archivo, variable): 
    ...
    """
    if type(variable) == str: #si solo es una variable
    ...
    elif type(variable) == list: #si son varias variables/columnas
        lista_variables = ["fecha"] + variable
        data = pd.read_csv(os.path.join(ruta_a_archivos,nombre_archivo), sep="\t", index_col = ["fecha"], parse_dates = True, usecols = lista_variables, na_values = r"\N") 
